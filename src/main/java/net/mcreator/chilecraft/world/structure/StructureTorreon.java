...
package net.mcreator.chilecraft.world.structure;
...
public class StructureTorreon extends ElementsChileanCraftMod.ModElement {
	public StructureTorreon(ElementsChileanCraftMod instance) {
		super(instance, 747);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 1397) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("chilean_craft", "torre12"));
				if (template == null)
					return;
