...
public class StructureAvion extends ElementsChileanCraftMod.ModElement {
	public StructureAvion(ElementsChileanCraftMod instance) {
		super(instance, 695);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 500) {
			...
			for (int a = 0; a < count; a++) {
				...
				require = Blocks.GRAVEL.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
